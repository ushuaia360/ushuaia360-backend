...
@trails_bp.route("/trails/<trail_id>/media/<media_id>", methods=["PUT", "PATCH"])
@require_admin
async def update_trail_media(trail_id: str, media_id: str, user_id: str):
    """Actualizar media del sendero (ej. cambiar media_type)"""
    try:
        trail_uuid = uuid.UUID(trail_id)
        media_uuid = uuid.UUID(media_id)
    except ValueError:
    ...
    values = [media_uuid, trail_uuid] + list(updates.values())
